        ...
        firestore()
            ...
            .then(querySnapshot => {
                ...
                querySnapshot.forEach(res => {
                    const data = res.data()
                    if(data?.Status==="In Processing"&&data?.Details?.Service_Type===service){
                        dummy.push(data)}
                });
                setData(dummy)
            }).catch(err => {
    ...
    return (
        <View style={{flex: 1, backgroundColor: Color.primary}}>
            <View style={{paddingHorizontal: 20, marginVertical: 40, flexDirection: "row", alignItems: "center"}}>
                <HeaderComponent back={false} title={"Active Orders"}/>
            </View>
            <View style={{
                flex: 1,
